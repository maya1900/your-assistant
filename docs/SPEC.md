...
| F13 | 测试连接 | 设置面板内一键发非流式 ping，验证 Base URL / Key / Model 是否齐活，展示延迟与上游错误 |
| F14 | 移动端适配 | < 768px 视口下侧边栏改为抽屉式（汉堡按钮 + slide-in + 半透明遮罩点击关闭），对话框全屏化，输入区/标题栏的快捷键提示与文字标签隐藏只留图标 |
...
| 断点 | 视口宽度 | 行为 |
|---|---|---|
| `< sm` | < 640px | 设置/System Prompt 对话框全屏，无圆角；输入区底部"上下文 / Temperature / Enter 提示"压缩为只显示必要 |
| `< md` | < 768px | 侧边栏改抽屉：默认隐藏，顶栏左侧出现汉堡按钮，点击 slide-in，半透明遮罩点击关闭；选中会话 / 新建会话 / 打开设置后自动关闭 |
| `≥ md` | ≥ 768px | 侧边栏 260px 固定在左 |

通用约束：

- 触摸热区 ≥ 36×36 px（接近 WCAG AAA 的 44px 推荐值，受限于设计精致度做了折衷）
- 空状态主标题字号：`< md` 时缩到 40px，`md+` 保持 56px
- 消息气泡 `max-w` 仍按百分比，移动端自然变窄
